...
* At first the email smtp did not work, this issue was due to that Google had changed their policy regarding secure passwords, in order to use gmail as a smpt service for Django I hade to log in to my google account and then go to "security" , then I need to add 2 factor authentication and add an application password. After adding application password I copied the newly created key and added in that as a enviorment variable in Heroku called SECRET_PASS and the key was the newly created application password. After that the bug was solved and everytime the user clicked forget password a link was sent.
...
And on settings.py file line 121 and 155 "Line too long".
